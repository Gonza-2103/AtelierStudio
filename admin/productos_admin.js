...
function mostrarProductos() {
    ...
    for (let i = 0; i < productos.length; i++) {
        ...
        if (producto.stockCritico !== "" && producto.stock <= producto.stockCritico) {
            mensajeStock = producto.stock + " - Stock crítico";
        }
        ...
        filaProducto.innerHTML = `
            ...
            <td class="acciones_tabla_admin">
                <button type="button" class="btn_mostrar_producto"data-id="${producto.id}">Mostrar</button>
                <button type="button" class="btn_editar_producto"data-id="${producto.id}">Editar</button>
                <button type="button" class="btn_eliminar_producto" data-id="${producto.id}">Eliminar</button>
            </td>`;
...
function conectarBotonesProductos() {
    const botonesMostrar = document.querySelectorAll(".btn_mostrar_producto");
    const botonesEditar = document.querySelectorAll(".btn_editar_producto");
    const botonesEliminar = document.querySelectorAll(".btn_eliminar_producto");

    for (let i = 0; i < botonesMostrar.length; i++) {
        botonesMostrar[i].addEventListener("click", function () {
                const idProducto = Number(botonesMostrar[i].dataset.id);
    ...
    for (let i = 0; i < botonesEditar.length; i++) {
        botonesEditar[i].addEventListener("click", function () {
                const idProducto = Number(botonesEditar[i].dataset.id);
    ...
    for (let i = 0; i < botonesEliminar.length; i++) {
        botonesEliminar[i].addEventListener("click", function () {
                const idProducto = Number(botonesEliminar[i].dataset.id);
...
function mostrarDetalleProducto(idProducto) {
    ...
    localStorage.setItem("productoSeleccionadoAdmin", JSON.stringify(producto));
    window.location.href = "producto_detalle.html";
}

// Guardar el identificador del producto que se editará
function editarProducto(idProducto) {
    localStorage.setItem("productoEditarId", idProducto);
...
function eliminarProducto(idProducto) {
    const producto = buscarProducto(idProducto);
    const confirmarEliminacion = confirm("¿Desea eliminar el producto " + producto.nombre + "?");
